<template>
  <div class="p-4 rounded-2xl bg-kros-surface dark:bg-[#111112] border border-kros-outline dark:border-[#1F1F21] overflow-x-auto overflow-visible relative group hover:border-kros-blue/5 transition-all min-h-[400px]">
    <table class="w-full text-left border-separate border-spacing-y-2.5">
      <thead>
        <tr class="text-[10px] font-bold uppercase tracking-[0.2em] text-kros-text/60 dark:text-white/60">
          <th class="px-6 py-4">Status</th>
          <th class="px-6 py-4">Empresa / Parceiro</th>
          <th class="px-6 py-4">Plano Ativo</th>
          <th class="px-6 py-4">Faturamento (MRR)</th>
        ...
        <tr v-for="company in sortedCompanies" :key="company.id" 
          ...
          <td class="px-6 py-5 first:rounded-l-2xl">
            <div class="flex items-center gap-2">
              <span :class="['w-2 h-2 rounded-full ring-4', company.is_active ? 'bg-emerald-500 ring-emerald-500/10' : 'bg-red-500 ring-red-500/10']"></span>
              <span :class="['text-[10px] font-bold uppercase tracking-tighter', company.is_active ? 'text-emerald-500' : 'text-red-500']">
                {{ company.is_active ? 'Ativa' : 'Inativa' }}
              </span>
            </div>
          </td>
          <td class="px-6 py-5">
            <h4 class="font-semibold text-sm tracking-tight text-white uppercase underline decoration-kros-blue/20 underline-offset-4">
              {{ company.name }}
            </h4>
            ...
            <p v-if="company.representative_name" class="text-[10px] font-medium text-white/60 mt-2 uppercase tracking-tighter">
              Rep: {{ company.representative_name }}
            </p>
            <p class="text-[10px] font-medium text-white/40 mt-1 uppercase tracking-widest italic">Desde {{ formatDate(company.created_at) }}</p>
          </td>
          <td class="px-6 py-5">
            <div class="flex flex-col gap-1.5">
               <span class="text-[10px] font-bold bg-kros-blue/10 text-kros-blue px-3 py-1.5 rounded-lg border border-kros-blue/20 uppercase tracking-tighter w-fit text-center">
                 {{ company.plan_name || 'Individual' }}
               </span>
               <span class="text-[9px] font-bold opacity-60 text-white uppercase tracking-[0.1em] ml-1">
                 • {{ company.billing_cycle || 'Mensal' }}
               </span>
            </div>
          </td>
          <td class="px-6 py-5">
             <span class="font-bold text-sm tracking-tighter text-white/90">{{ formatCurrency(company.monthly_price || 0) }}</span>
          </td>
          <td class="px-6 py-5 last:rounded-r-2xl text-right">
             <div class="flex items-center justify-end gap-3 opacity-0 group-hover/row:opacity-100 transition-all">
                <button @click="onEdit(company)" class="p-2.5 rounded-xl bg-kros-text/5 dark:bg-white/5 text-kros-text dark:text-white hover:bg-kros-blue hover:text-white transition-all">
                ...
                <button @click="onDelete(company.id)" class="p-2.5 rounded-xl bg-kros-text/5 dark:bg-white/5 text-red-500 hover:bg-red-500 hover:text-white transition-all">
