       ...
       <div v-for="plan in plans" :key="plan.id" class="p-6 rounded-3xl bg-kros-surface dark:bg-[#111112] border border-kros-outline dark:border-[#1F1F21] group hover:border-kros-blue/20 transition-all flex flex-col h-full relative overflow-hidden">
          ...
          <div class="flex-1 z-10">
            ...
            <div class="mt-4 mb-6">
              ...
              <span class="text-[9px] font-bold uppercase tracking-widest text-white/40 ml-1">/ {{ (plan.billing_cycle || 'mensal').toLowerCase() }}</span>
            </div>
...
const handleDelete = async (id: string) => {
  if (confirm('Tem certeza que deseja excluir esse plano? Isso não afetará empresas que já o utilizam, mas impedirá novas associações.')) {
    try {
      await deletePlan(id)
    } catch (err) {
