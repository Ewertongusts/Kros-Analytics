  ...
  <div class="space-y-4">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-6">
      ...
      >
        ...
        <option value="created" class="bg-[#111112] text-white">Criações</option>
        <option value="updated" class="bg-[#111112] text-white">Edições</option>
        <option value="deleted" class="bg-[#111112] text-white">Exclusões</option>
        <option value="whatsapp_sent" class="bg-[#111112] text-white">Envios WhatsApp</option>
      </select>
        ...
        <div class="bg-white/[0.02] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-all">
          ...
          <div v-if="entry.metadata && Object.keys(entry.metadata).length > 0" class="mt-3 pt-3 border-t border-white/5">
            <div class="grid grid-cols-2 gap-3 text-xs">
              <div v-if="entry.metadata.product_name" class="space-y-1">
                <span class="text-[9px] font-bold text-white/40 uppercase tracking-widest">Produto</span>
                <p class="text-white/70 font-medium">{{ entry.metadata.product_name }}</p>
              </div>
              ...
              <div v-if="entry.metadata.customer_name" class="space-y-1">
                <span class="text-[9px] font-bold text-white/40 uppercase tracking-widest">Cliente</span>
                <p class="text-white/70 font-medium">{{ entry.metadata.customer_name }}</p>
              </div>
            </div>
          </div>
        </div>
...
const filteredHistory = computed(() => {
  ...
  if (filterType.value !== 'all') {
    filtered = filtered.filter(h => h.action_type === filterType.value)
  }
...
const getActionIcon = (type: string) => {
  const icons: Record<string, any> = {
    ...
    updated: () => h('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': '2.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, [
    ...
    deleted: () => h('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': '2.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, [
      h('path', { d: 'M3 6h18M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2' })
    ]),
    whatsapp_sent: () => h('svg', { xmlns: 'http://www.w3.org/2000/svg', viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', 'stroke-width': '2.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, [
