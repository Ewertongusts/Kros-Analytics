<template>
  <tr class="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
    ...
    <td class="px-6 py-4">
      <div>
        <p class="text-[10px] text-white/30 uppercase tracking-wider mb-1">Criação</p>
        <p class="text-xs text-white/60">{{ formatDate(sale.created_at) }}</p>
        <p v-if="sale.created_by_name" class="text-[9px] text-white/30 mt-0.5">Por: {{ sale.created_by_name }}</p>
      </div>
      <div v-if="sale.last_receipt_sent_at" class="mt-3 pt-3 border-t border-white/5">
        <div class="flex items-center gap-1.5 mb-1">
          <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 448 512" fill="currentColor" class="text-white/40">
          ...
          <p class="text-[10px] text-white/40 uppercase tracking-wider">Último envio</p>
        </div>
        <p class="text-xs text-white/60">{{ formatDateTime(sale.last_receipt_sent_at) }}</p>
      </div>
    </td>
